(function () {
  ...
  function grantAccess() {
    ...
    uniqueEmails.forEach(function (email) {
      ...
      const idx = privileges.findIndex(function (p) {
        return (
          p.grantedByTeacherId === session.id &&
          normalizeEmail(p.studentEmail) === email
        );
      });

      if (idx > -1) {
        const merged = Array.from(
          new Set([].concat(privileges[idx].permissions || [], perms)),
        );
        ...
        privileges.push({
          id:
            "priv_" + Date.now() + "_" + Math.random().toString(36).slice(2, 6),
    ...
    grantEmailsEl.value = "";
    document
      .querySelectorAll('.check-grid input[type="checkbox"]')
  ...
  function updateSuspension(suspend) {
    ...
    const payload = {
      id:
        idx > -1
          ? flags[idx].id
          : "flag_" + Date.now() + "_" + Math.random().toString(36).slice(2, 6),
      studentEmail: email,
      suspended: suspend,
      reason:
        reason || (suspend ? "Suspended by teacher" : "Restored by teacher"),
  ...
  function renderPrivileges() {
    ...
    const mine =
      session && session.id
        ? all.filter(function (p) {
            return p.grantedByTeacherId === session.id;
          })
        : [];
    ...
    grantListEl.innerHTML = mine
      .map(function (entry) {
        const tags = (entry.permissions || [])
          .map(function (perm) {
        ...
        return (
          ...
          escapeHtml(entry.studentEmail) +
          "</strong>" +
          '<div class="tag-group">' +
          tags +
          "</div>" +
          "</div>" +
          '<button class="btn danger" data-revoke="' +
          entry.id +
          '"><i class="fa-solid fa-xmark"></i> Revoke</button>' +
          "</article>"
        );
      })
      .join("");
  ...
  function renderPenalties() {
    ...
    const mine =
      session && session.id
        ? all.filter(function (p) {
            return p.issuedByTeacherId === session.id;
          })
        : [];
    ...
    if (!mine.length) {
      penaltyListEl.innerHTML =
        '<p class="muted-empty">No penalties logged yet.</p>';
      return;
    }

    penaltyListEl.innerHTML = mine
      .map(function (entry) {
        ...
        return (
          ...
          "</span>" +
          "</div>" +
          '<button class="btn ok" data-clear-penalty="' +
          entry.id +
          '"><i class="fa-solid fa-check"></i> Clear</button>' +
          "</article>"
        );
      })
      .join("");
    ...
    penaltyListEl
      .querySelectorAll("[data-clear-penalty]")
  ...
  function renderSuspensions() {
    ...
    if (!active.length) {
      suspendListEl.innerHTML =
        '<p class="muted-empty">No active suspensions.</p>';
      return;
    }

    suspendListEl.innerHTML = active
      .map(function (entry) {
        ...
        return (
          ...
          "</strong>" +
          '<span class="row-sub">' +
          escapeHtml(entry.reason || "Suspended") +
          " | " +
          escapeHtml(when) +
          "</span>" +
          "</div>" +
          '<button class="btn ok" data-restore="' +
          escapeHtml(entry.studentEmail) +
          '"><i class="fa-solid fa-unlock"></i> Restore</button>' +
          "</article>"
        );
      })
      .join("");
