...
const demoTopicCards = document.querySelectorAll('.demo-topic-card');
...
const DEMO_QUERY_TOPIC_KEY = 'demoTopic';
const DEMO_QUERY_AUTO_KEY = 'autoDemo';
...
demoTopicCards.forEach((card) => {
  card.addEventListener('click', () => {
    const topicId = card.dataset.topic || 'data-modeling-demo-v1';
    if (demoTopicSelect) demoTopicSelect.value = topicId;
    activateDemoMode(topicId);
  });
});

applyDemoQueryOptions();
...
async function uploadFiles() {
  ...
  let aiProcessingTimer = setTimeout(() => {
    if (uploadBtn.disabled) {
      feedback.textContent = 'AI is analysing your slides and generating quiz questions...';
      if (aiStatusEl) aiStatusEl.classList.add('loading');
    }
  }, 1800);
  ...
  try {
    ...
    if (aiStatusEl) aiStatusEl.classList.remove('loading');
...
function setActiveDemoCard(topicId) {
  demoTopicCards.forEach((card) => {
    card.classList.toggle('active', card.dataset.topic === topicId);
  });
}

function activateDemoMode(topicId) {
  const selectedTopicId = topicId || (demoTopicSelect ? demoTopicSelect.value : 'data-modeling-demo-v1');
  const demoTopic = DEMO_TOPICS[selectedTopicId] || DEMO_TOPICS['data-modeling-demo-v1'];

  if (demoTopicSelect) {
    demoTopicSelect.value = demoTopic.id;
  }
  setActiveDemoCard(demoTopic.id);
...
function applyDemoQueryOptions() {
  const params = new URLSearchParams(window.location.search);
  const requestedTopic = params.get(DEMO_QUERY_TOPIC_KEY);
  const autoDemo = params.get(DEMO_QUERY_AUTO_KEY);

  if (requestedTopic && DEMO_TOPICS[requestedTopic] && demoTopicSelect) {
    demoTopicSelect.value = requestedTopic;
    setActiveDemoCard(requestedTopic);
  }

  if (autoDemo === '1' || autoDemo === 'true') {
    activateDemoMode(requestedTopic || undefined);
  } else if (demoTopicSelect) {
    setActiveDemoCard(demoTopicSelect.value);
  }
}
...
async function checkAiStatus() {
  try {
    ...
    if (aiStatusEl && aiStatusTextEl) {
      if (config.huggingFaceEnabled) {
        aiStatusEl.classList.add('active');
        aiStatusTextEl.textContent = `AI quiz generation active (Hugging Face: ${config.model || 'configured model'})`;
      } else {
        aiStatusTextEl.textContent = 'Basic quiz generation - add HUGGING_FACE_API_KEY to enable AI';
      }
    }
  } catch (_err) {
    if (aiStatusTextEl) aiStatusTextEl.textContent = 'Basic quiz generation (server offline)';
  }
}
