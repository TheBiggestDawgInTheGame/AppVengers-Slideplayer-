...
async function readFileText(fileObj) {
  ...
  // Binary formats — send to server for parsing
  if (serverFormats.includes(ext)) {
    try {
      const formData = new FormData();
      formData.append("files", fileObj.file, fileObj.name);
      const res = await fetch("http://localhost:3000/api/parse-files", { method: "POST", body: formData });
      const data = await res.json();
      if (res.ok && data.text) return data.text;
    } catch (e) {
      console.warn("Server parse failed for", fileObj.name, e.message);
    }
    return "[File: " + fileObj.name + " — server parsing unavailable]";
  }

  // Plain text files — FileReader
  return new Promise(function (resolve) {
    const textTypes = ["text/", "application/json", "application/xml"];
    const isText = textTypes.some(function (t) { return fileObj.type.startsWith(t); })
      || /\.(txt|md|csv|html|htm|xml|json|js|ts|py|java|c|cpp|css)$/i.test(fileObj.name);

    if (!isText) {
      resolve("[File: " + fileObj.name + "]");
      return;
    }
...
async function generateQuiz() {
  ...
  try {
    // Read text from all uploaded files
    ...
    try {
      usedRes = await fetch("http://localhost:3000/api/quiz/gemini", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ text: combinedText, count: questionCount })
      });
      data = await usedRes.json();
      if (!usedRes.ok || !data.questions || data.questions.length === 0) {
        throw new Error(data.error || "Gemini returned no questions");
      }
    } catch (geminiErr) {
      console.warn("Gemini quiz failed, falling back to HuggingFace:", geminiErr.message);
      source = "huggingface";
      usedRes = await fetch("http://localhost:3000/api/quiz/huggingface", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ text: combinedText, count: questionCount })
      });
    ...
    if (!data || !data.questions || data.questions.length === 0) {
      alert("Quiz generation failed: " + (data?.error || "No questions returned."));
      return;
    }

    // Save to localStorage for use in games / quiz runner
    // Use the canonical key all games read from
    localStorage.setItem("slidePlayGeneratedQuizData", JSON.stringify(data.questions));
    // Also save full metadata under the app-vengers key
