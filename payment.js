...
function validateCardFields() {
  ...
  if (!name || !name.value.trim()) { alert("Please enter the cardholder name."); return false; }
  if (!number || number.value.replace(/\s/g, "").length < 13) { alert("Please enter a valid card number."); return false; }
  if (!expiry || expiry.value.trim().length < 4) { alert("Please enter a valid expiry date."); return false; }
  if (!cvc || cvc.value.trim().length < 3) { alert("Please enter the CVC."); return false; }
  return true;
}
