...
const QUIZ_SLOW_START = 0.68; // start slowing when answer blocks are close enough to see clearly
const QUIZ_SLOW_END   = 0.94; // restore full speed after passing the near zone
...
function showQuizPrompt(question) {
  ...
  document.getElementById('quizQuestionText').textContent = question.prompt;
  document.getElementById('quizFeedback').textContent = 'Run into the correct block to answer.';
...
class QuizItem {
  constructor(lane, label, correct) {
    ...
    this.el.className = 'quiz-item ' + (correct ? 'correct' : 'wrong');
  ...
  update(speed = speedMult) {
    this.progress += 0.003 * speed;
    if (this.progress >= 1.05) {
      this.alive = false;
      return;
    }
    const scale = 0.14 + 0.86 * this.progress;
    ...
    this.el.style.top     = (cy - size / 2 - 10) + 'px';
    this.el.style.width   = size + 'px';
    this.el.style.height  = size + 'px';
    this.el.style.opacity = String(Math.min(1, 0.24 + 0.76 * this.progress));
...
function finishQuiz(correct) {
  ...
  feedback.textContent = correct ? 'Correct! Nice choice.' : 'Wrong answer. Try again next time.';
  if (correct) {
    correctQuizAnswers++;
    score += 25;
  } else {
...
function getQuizApproachSpeedMult() {
  if (!quizActive) return speedMult;
  const isApproaching = quizItems.some(item => item.progress >= QUIZ_SLOW_START && item.progress < QUIZ_SLOW_END);
  return isApproaching ? Math.min(speedMult, QUIZ_SLOW_SPEED) : speedMult;
}

function updateQuizItems(speed = speedMult) {
...
function gameLoop() {
  ...
  // Slow the game when quiz answer blocks are very close, so players can see the collision.
  const effectiveSpeed = getQuizApproachSpeedMult();

  // Advance road animation
  dashOff = (dashOff + 2.2 * effectiveSpeed) % 34;
  ...
  // Start a quiz when it's time
  if (!quizActive && frameCount >= nextQuizFrame) {
    triggerQuiz();
  }

  // Update quiz answer blocks and check player choice
  if (quizActive) {
    updateQuizItems(effectiveSpeed);
  }
