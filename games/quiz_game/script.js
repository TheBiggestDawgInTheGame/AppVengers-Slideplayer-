...
if (urlParams.get('source') === 'upload' && urlParams.get('legacy') !== '1') {
    window.location.replace('../jeopardy-3d/index.html?source=upload&game=quiz');
}
...
function initThreeJSScene() {
    ...
    const sceneHost = document.getElementById('threejs-scene');
    if (!sceneHost) return;
...
function showResults() {
    ...
    try {
        ...
        fetch('http://localhost:3000/api/leaderboard', {
            ...
            body: JSON.stringify({ game: 'quiz', name: playerName, score })
        }).catch(() => {});
    } catch (_e) {}
...
if (window.GameModes) {
    GameModes.init({
        gameLabel: 'Millionaire Quiz',
        startFn: startQuiz,
        resetFn: () => { stopTimer(); showScreen('startScreen'); },
        getScore: () => score
    });
}
